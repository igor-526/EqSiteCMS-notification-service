...
def test_before_send_removes_credentials_and_body() -> None:
    sanitized = sentry_module.before_send(
        {
            "request": {"headers": {"Cookie": "secret", "X-Service-Key": "secret"}, "data": "secret"},
            "extra": {"smtp_password": "secret"},
        },
        {},
    )
    assert sanitized == {
        "request": {"headers": {"Cookie": "[Filtered]", "X-Service-Key": "[Filtered]"}},
        "extra": {"smtp_password": "[Filtered]"},
    }
...
async def test_lifespan_closes_consumer_nats_database_and_metrics(
    ...
    monkeypatch.setattr(main, "wire_event_handlers", Mock())
    monkeypatch.setattr(main.container, "nats_client", Mock(return_value=nats))
    monkeypatch.setattr(main.container, "callback_request_consumer", Mock(return_value=consumer))
...
async def test_nats_startup_failure_is_not_masked_by_metrics_cleanup(
    ...
    monkeypatch.setattr(main, "wire_event_handlers", Mock())
    monkeypatch.setattr(main.container, "nats_client", Mock(return_value=nats))
    monkeypatch.setattr(main.container, "callback_request_consumer", Mock(return_value=Mock()))
    start_metrics = Mock()
