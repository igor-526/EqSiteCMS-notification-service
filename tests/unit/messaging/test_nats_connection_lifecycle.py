...
def make_client(
    ...
    client = NatsJetstreamClient(settings or NatsSettings())
...
async def test_close_survives_drain_while_reconnecting(caplog: pytest.LogCaptureFixture) -> None:
    client, connection = make_client(drain_error=nats.errors.ConnectionReconnectingError())
...
async def test_close_survives_drain_timeout() -> None:
    client, connection = make_client(drain_error=TimeoutError())
...
async def test_close_does_not_swallow_unrelated_errors() -> None:
    client, _ = make_client(drain_error=RuntimeError("boom"))
...
async def test_consumer_setup_retries_until_owner_creates_stream(
    ...
    monkeypatch.setattr(client, "_add_callback_requested_consumer", flaky)
...
async def test_consumer_setup_fails_after_exhausting_attempts(
    ...
    monkeypatch.setattr(client, "_add_callback_requested_consumer", always_missing)

    with caplog.at_level(logging.ERROR), pytest.raises(NotFoundError):
        await client.setup_consumers()

    assert attempts["count"] == 3
    assert settings.nats_stream_site_events in caplog.text
...
async def test_consumer_setup_does_not_retry_configuration_errors(
    ...
    monkeypatch.setattr(client, "_add_callback_requested_consumer", bad_config)
...
async def test_service_never_creates_foreign_site_events_stream() -> None:
    settings = NatsSettings()
    client = NatsJetstreamClient(settings)
    jetstream = MagicMock()
    jetstream.add_stream = AsyncMock()
    client._jetstream = jetstream
    client._connection = MagicMock(is_connected=True)

    await client.setup_streams()

    created = [call.kwargs["config"].name for call in jetstream.add_stream.await_args_list]
    assert created == [settings.nats_stream_notification_commands]
    assert settings.nats_stream_site_events not in created
