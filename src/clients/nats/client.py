...
class NatsJetstreamClient:
    ...
    async def _setup_callback_requested_consumer(self) -> None:
        """
        Durable регистрируется на stream `SITE_EVENTS`, которым владеет backend.
        Если владелец ещё не создал stream, JetStream отвечает 404; это штатная
        гонка деплоя, поэтому попытка повторяется с backoff, а не роняет startup.
        """
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                await self._add_callback_requested_consumer()
                return
                ...
                if attempt >= max_attempts:
                    logger.error(
                        ...
                        stream,
                        max_attempts,
                        self._settings.nats_consumer_callback_requested,
                    )
                    raise
    ...
    async def _add_callback_requested_consumer(self) -> None:
        jetstream = self._get_jetstream()
